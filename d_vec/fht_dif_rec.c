...
*/
void
_d_vec_fht_dif_rec(double * in, double * stab, long step, long n)
{
    long nh, i, send;
    double t;
    ...
    {
        _d_vec_fht_dif_iter(in, stab, step, n);
        return;
    }
    ...
    if (step == 4)
    {
        _d_vec_fht_dif_rec(in     , stab + send, 1, nh);
        _d_vec_fht_dif_rec(in + nh, stab + send, 1, nh);
    } 
    else 
    {
        _d_vec_fht_dif_rec(in     , stab, 2*step, nh);
        _d_vec_fht_dif_rec(in + nh, stab, 2*step, nh);
