...
*/
slong _fmpz_mpoly_mul_heap_part1B(fmpz ** poly1, ulong ** exp1, slong * alloc,
...
/*
    ...
    "start" and "end" are not monomials but arrays of indicides into exp3
*/
slong _fmpz_mpoly_mul_heap_partB(fmpz ** poly1, ulong ** exp1, slong * alloc,
                 const fmpz * poly2, const ulong * exp2, slong len2,
                 const fmpz * poly3, const ulong * exp3, slong len3,
            slong * start, slong * end, slong N, ulong maskhi, ulong masklo)
{
    ...
    if (N == 1)
        return _fmpz_mpoly_mul_heap_part1B(poly1, exp1, alloc,
                                          poly2, exp2, len2,
                                          poly3, exp3, len3,
                                                    start, end, maskhi);
...
typedef struct
{
    slong idx;
    slong time;
    mul_heap_threadedB_base_t * basep;
    mul_heap_threadedB_div_t * divp;
    volatile slong stage;
    pthread_mutex_t mutex;
    pthread_cond_t cond;
}
mul_heap_threadedB_arg_t;




void * _fmpz_mpoly_mul_heap_threadedB_worker(void * arg_ptr)
{
    mul_heap_threadedB_arg_t * arg = (mul_heap_threadedB_arg_t *) arg_ptr;
/*    mul_heap_threadedB_arg_t *parg = (mul_heap_threadedB_arg_t *) arg_ptr - 1;*/
    mul_heap_threadedB_div_t * divs;
    mul_heap_threadedB_base_t * base;
    slong i, j, * dummy;
/*
    timeit_t time, time2;
*/
...
*/
        if (i > 0)
        {
            divs[i].len1 = _fmpz_mpoly_mul_heap_partB(
...
*/
slong _fmpz_mpoly_mul_heap_threadedB(fmpz ** poly1, ulong ** exp1, slong * alloc,
                 const fmpz * coeff2, const ulong * exp2, slong len2,
                 const fmpz * coeff3, const ulong * exp3, slong len3,
...
/*
    timeit_t time;
*/
    base = flint_malloc(sizeof(mul_heap_threadedB_base_t));
    base->nthreads = flint_get_num_threads();
    base->ndivs    = base->nthreads*4;
    base->coeff2 = coeff2;
    base->exp2 = exp2;
    base->len2 = len2;
    base->coeff3 = coeff3;
...
*/
    ...
    for (i = 0; i < base->nthreads; i++)
    {
        ...
        args[i].basep = base;
        args[i].divp = divs;
        pthread_create(&threads[i], NULL, _fmpz_mpoly_mul_heap_threadedB_worker, &args[i]);
    }
    for (i = 0; i < base->nthreads; i++)
    {
...
*/
    ...
    k = 0; /* avoid uninitialized warning */
    for (i = base->ndivs - 1; i >= 0; i--)
    {
/*
        flint_printf("div %wd thread %wd time %wd\n",i,divs[i].threadidx,divs[i].time);
*/
...
void fmpz_mpoly_mul_heap_threadedB(fmpz_mpoly_t poly1, const fmpz_mpoly_t poly2,
                          const fmpz_mpoly_t poly3, const fmpz_mpoly_ctx_t ctx)
{
    ...
    if (poly1 == poly2 || poly1 == poly3)
    {
        ...
        if (poly2->length >= poly3->length)
            len = _fmpz_mpoly_mul_heap_threadedB(
