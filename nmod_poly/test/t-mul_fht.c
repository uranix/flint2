...
main(void)
{
    ...
    for (i = 0; i < 20000; i++)
    {
        ...
        mp_limb_t n = n_randbits(state, n_randint(state, 20) + 1);
        if (n == 0) n += 2;
        ...
        nmod_poly_init(b, n);
        nmod_poly_init(c, n);
        nmod_poly_randtest(b, state, n_randint(state, 2000));
        nmod_poly_randtest(c, state, n_randint(state, 2000));
       
        nmod_poly_mul_classical(a1, b, c);
        nmod_poly_mul_fht(a2, b, c, 0);
